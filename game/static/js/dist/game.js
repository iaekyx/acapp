...
class AcGameObject {
    constructor(){
        ...
        this.timedelta = 0; // 当前帧距离上一帧的时间间隔 因为不同浏览器的每秒帧数可能不同，需要用时间来衡量
    }
    start(){  //    只会在第一帧执行一次
    }
    update(){ //  每一帧都会执行一次
...
class Player extends AcGameObject{
    ...
    update_move(){
        ...
        if(this.damage_speed > this.eps){
            ...
            this.x += this.damage_y * this.damage_speed * this.timedelta /
1000;
...
class Settings{
    ...
    getinfo_acapp(){
        let outer = this;
        $.ajax({
            url:"https://app5757.acapp.acwing.com.cn/settings/acwing/acapp/apply_code/",
            type:"GET",
            success:function(resp){
                if(resp.result === "success"){
